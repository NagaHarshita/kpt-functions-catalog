...
List setter values on resource fields. Setters serve as parameters for template-free
...
We use ConfigMap to configure the `list-setters` function. The desired setter
...
```shell
$ kpt fn eval --image gcr.io/kpt-fn/list-setters:unstable --fn-config ./list-setters-fn-config
```

Alternatively, setter values can be passed as key-value pairs in the CLI

```shell
$ kpt fn eval --image gcr.io/kpt-fn/list-setters:unstable -- 'image=ubuntu' 'replicas=3'
```
